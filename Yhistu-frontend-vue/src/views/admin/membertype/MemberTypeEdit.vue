...
@Options({
  components: {},
  props: { memberType: Object as () => IMemberType },
  emits: [],
})
export default class MemberTypeEdit extends Vue {
  ...
  async created() {
    ...
    this.description = this.memberType.description;
    this.isMemberOfBoard = this.memberType.isMemberOfBoard;
    this.isAdministrator = this.memberType.isAdministrator;
    this.isRegularMember = this.memberType.isRegularMember;
    this.isManager = this.memberType.isManager;
    this.isAccountant = this.memberType.isAccountant;
  }

  async editClicked() {
    const res = await this.memberTypeService.update({
    ...
    if (res.status >= 300) {
      ...
      const element = "#editMemberType-" + this.memberType.id;
...
<template>
  <div class="container">
    <h3 class="mb-3 mt-2 text-center">Edit member type</h3>
