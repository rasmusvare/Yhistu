...
@Options({
  components: {},
  props: { contactTypeId: String },
  emits: [],
})
export default class ContactTypeDelete extends Vue {
  ...
  async deleteClicked() {
    const res = await this.contactTypeService.remove(this.contactTypeId);
    if (res.status >= 300) {
      ...
      const element = "#removeContactType-" + this.contactTypeId;
...
<template>
  <div class="container">
    <h5 class="mb-4 mt-2 text-center">Delete contact type</h5>
        ...
        <div>
          <div class="form-floating mb-3">
            <p class="mb-4 mt-2 text-center">
              This will delete the contact type from the association. Please make
              sure no members are using this contact type before deleting.
            </p>
            <p class="mb-4 mt-2 text-center">
